...
class UpFIRDownTests: XCTestCase {
    ...
    func runTest(_ P: Int, _ h: [Float], _ Q: Int, _ x: [Float], _ y: [Float], accuracy:Float=2.0e-7) {
        ...
        let f = UpFIRDown(source:SourceBox<RealSamples>.NilReal(), P, h, Q)
        var o=RealSamples()
        f.process(RealSamples(x), &o)
        AssertEqual(o, y, accuracy:accuracy)
        
        // Test on two halves in sequence, to exercise stream overalap
        let f2 = UpFIRDown(source:SourceBox<RealSamples>.NilReal(), P, h, Q)
        let half = x.count / 2
        ...
        let f3 = UpFIRDown(source:SourceBox<RealSamples>.NilReal(), P, h, Q)
        oo=RealSamples()
        var o3 = RealSamples()
        for i in 0..<x.count {
    ...
    func runTest(_ P: Int, _ h: [Float], _ Q: Int, _ x: [DSPComplex], _ y: [DSPComplex], accuracy:Float=2.0e-7) {
        ...
        // Test on the whole block
        let f = UpFIRDown(source:SourceBox<ComplexSamples>.NilComplex(), P, h, Q)
        var o=ComplexSamples()
        f.process(ComplexSamples(x), &o)
        AssertEqual(o, y, accuracy:accuracy)
        
        // Test on two halves in sequence, to exercise stream overalap
        let f2 = UpFIRDown(source:SourceBox<ComplexSamples>.NilComplex(), P, h, Q)
        let half = x.count / 2
        ...
        // Test on one sample at a time, to exercise stream overalap
        let f3 = UpFIRDown(source:SourceBox<ComplexSamples>.NilComplex(), P, h, Q)
        oo=ComplexSamples()
    ...
    func runIdentityTest(_ up:Int, _ down:Int, _ P:Int, _ N:Int) {
        let b = UpFIRDown(source:SourceBox<RealSamples>.NilReal(),
