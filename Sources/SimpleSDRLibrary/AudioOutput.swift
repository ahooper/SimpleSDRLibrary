...
public class AudioOutput:SinkProtocol {
    public typealias Input = RealSamples
    var source:SourceBox<Input>?
    ...
    var audioBuffers = [ContiguousArray<DataType>(),
                        ContiguousArray<DataType>()] // used instead of BufferedStage output
    ...
    public init() {
        ...
        sampleRate = Int(nominalSampleRate)
    ...
    public func setSource<S:SourceProtocol>(source:S) where S.Output == Input {
        self.source = SourceBoxHelper(source)
        source.addSink(self, asThread:false)
    }
    ...
    public func process(_ input: Input) {
        ...
        for i in 0..<input.count {
            ...
            let v = input[i]
            //TODO: multiple channels for stereo
    ...
    private func filledBuffer(_ filling: Int) {
        //print("AudioOutput filledBuffer",filling,fillPoint,audioBuffers[filling][0],audioBuffers[filling][fillPoint-1],deviceStarted,deviceStopped)
        if underflowCount > 0 {
    ...
    private static let outputCallback:AudioDeviceIOProc = {
        ...
        */
        ...
        let s = Unmanaged<AudioOutput>.fromOpaque(inClientData!).takeUnretainedValue()
        ...
        //print("AudioOutput", inDevice, "callback", s.callbackPlaying, numSamples, numToCopy, s.audioBuffers[s.callbackPlaying][s.callbackPlayPoint])
        ...
        if s.callbackPlayPoint == s.audioBuffers[s.callbackPlaying].count {
            s.callbackPlaying = 1 - s.callbackPlaying
    ...
    private static let propertyListener:AudioObjectPropertyListenerProc = {
        ...
        */
        ...
        let s = Unmanaged<AudioOutput>.fromOpaque(callbackContext!).takeUnretainedValue()
    ...
    func getSampleRate(_ deviceID: AudioDeviceID) -> Float {
        ...
        return Float(nominalSampleRate)
    }
    ...
    func setSampleRate(_ deviceID: AudioDeviceID, _ rate: Float) {
