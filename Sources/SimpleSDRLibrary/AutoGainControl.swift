...
public class AutoGainControl<Samples:DSPSamples>:BufferedStage<Samples,Samples> {
    var gain:Float
    var isLocked:Bool
    var ceil, ceilMA, ceilMAA:Float

    public init<S:SourceProtocol>(source:S?, gain:Float=0.5) where S.Output == Samples {
        self.gain = gain
        ceil = 1
        ceilMA = 1
        ceilMAA = 1
        isLocked = false
        super.init("AutoGainControl", source:source)
    }
