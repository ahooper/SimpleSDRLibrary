...
public class DCRemove<Samples:DSPSamples>:BufferedStage<Samples,Samples> {
    private var ma1, ma2:MovingAverage<Samples>
    
    public init<S:SourceProtocol>(source:S?, _ D:Int) where S.Output == Samples {
        ma1 = MovingAverage<Samples>(D)
        ma2 = MovingAverage<Samples>(D)
        super.init("DCRemove", source:source)
    }
