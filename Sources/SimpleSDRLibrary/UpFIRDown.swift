...
public class UpFIRDown<Samples:DSPSamples>:BufferedStage<Samples,Samples> {
    ...
    public init<S:SourceProtocol>(source:S?,
        ...
        super.init("UpFIRDown", source:source)
    }
    ...
    override public func sampleFrequency() -> Double {
        if let source = source { return source.sampleFrequency() * Double(up) / Double(down) }
        else { return Double.signalingNaN }
    }
    ...
    override public func process(_ x:Samples, _ out:inout Samples) {
        ...
        while i < inCount {
            ...
            if i < Qminus1 {
                ...
                out[y] = overlap.weightedSum(at:i, polyphase[p])
            } else {
                out[y] = x.weightedSum(at:i-Qminus1, polyphase[p])
            }
            y += 1
            p += down
            let adv = p / up
